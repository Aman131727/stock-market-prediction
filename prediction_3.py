...
df['Upper_Band'] = df['SMA'] + (df['Close'].rolling(window=20).std() * 2)
df['Lower_Band'] = df['SMA'] - (df['Close'].rolling(window=20).std() * 2)
...
model = Sequential()
model.add(LSTM(128, return_sequences=True, input_shape=(time_step, 1)))  # Increased units
model.add(Dropout(0.3))  # Increased dropout to prevent overfitting
model.add(LSTM(128, return_sequences=False))
model.add(Dense(64, activation='relu'))  # Added more dense layers
model.add(Dense(1))

model.compile(optimizer='adam', loss='mean_squared_error')
model.fit(X_train, Y_train, batch_size=32, epochs=30)  # Increased batch size & epochs

# Random Forest Model
rf_model = RandomForestRegressor(n_estimators=200, random_state=42)  # Increased estimators
...
# Plot LSTM predictions with Bollinger Bands
...
plt.plot(df.index[-len(predictions):], df['Upper_Band'][-len(predictions):], label="Upper Bollinger Band", color='green', linestyle='--')
plt.plot(df.index[-len(predictions):], df['Lower_Band'][-len(predictions):], label="Lower Bollinger Band", color='blue', linestyle='--')
plt.xlabel("Date")
plt.ylabel("Stock Price")
plt.title("Reliance Industries Stock Price Prediction with Bollinger Bands")
plt.legend()
plt.show()
